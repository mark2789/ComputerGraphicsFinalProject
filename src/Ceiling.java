...
public class Ceiling extends Shape3D {
	int scale = 20;
	float smallWidth  = -0.93f;;
	float length = 6;
	public  Ceiling() {
	}
	public Ceiling(float smallWidth) {
		this.smallWidth = smallWidth;
	}
	public Ceiling(float smallWidth, float length) {
		this.smallWidth = smallWidth;
		this.length = length;
	}
	Shape3D createCeiling() {
		...
		TexCoord2f[] tex = {new TexCoord2f(0, 0f), new TexCoord2f(0, 0.5f), new TexCoord2f(0.5f, 0f), new TexCoord2f(0.5f, 0.50f)};
	...
	Shape3D createCeilingSmall() {
		...
		floor.setCoordinate(2, new Point3f(-1.3f * scale, 0f * scale, length / 4f * scale));
		floor.setCoordinate(3, new Point3f(smallWidth * scale, 0f * scale, length / 4f * scale));
		
		floor.setCoordinate(4, new Point3f(smallWidth * scale, 0f * scale, length / 2f * scale));
		floor.setCoordinate(5, new Point3f(-1.3f * scale, 0f * scale, length / 2f * scale));
		
		floor.setCoordinate(6, new Point3f(smallWidth * scale, 0f * scale, length * 3f / 4f * scale));
		floor.setCoordinate(7, new Point3f(-1.3f * scale, 0f * scale, length * 3f / 4f * scale));
		
		floor.setCoordinate(8, new Point3f(smallWidth * scale, 0f * scale, length * scale));
		floor.setCoordinate(9, new Point3f(-1.3f * scale, 0f * scale, length * scale));
